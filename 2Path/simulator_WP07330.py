...
f.write('Number of Agents:'+str(Num_Agents)+'\n')
...
for obstFeature in obstFeatures:
    ...
    wall.pointer1 = np.array([float(obstFeature[7]), float(obstFeature[8])])
    wall.pointer2 = np.array([float(obstFeature[9]), float(obstFeature[10])])
    walls.append(wall)
...
[Num_Doors, Num_DoorFeatures] = np.shape(doors)
...
while running:
	...
	#int(ai.radius*ZOOMFACTOR), AGENTSICKNESS)
        ...
        if SHOWVELOCITY:
            ...
            pygame.draw.line(screen, tan, scPos, endPosDV, 2)
        ...
        for idaj, agentOther in enumerate(agents):
            ...
            if comm[idai, idaj] == 1 and SHOWINTELINE: 
                pygame.draw.line(screen, purple, scPos, scPosOther, 2)
