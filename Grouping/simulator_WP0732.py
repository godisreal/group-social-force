...
while running:
	...
	#ai.p = 1 - ai.ratioV  	# Method-2 
		...
		peopleInter += ai.agentForce(aj)*anisoF
	     ...
	     #################################
	     if dij < ai.B_CF*BFactor[idai, idaj] + rij*DFactor[idai, idaj] and no_wall_flag_ij:
	...
	for idwall, wall in enumerate(walls):
	    result, flag, diw = ai.wallOnRoute(wall)
	    if flag: 
	...
	if flag:
	    if np.dot(wallDirection, ai.desiredV) < 0.0:
		wallDirection = -wallDirection
	    
	    ai.direction = ai.direction + wallDirection/np.linalg.norm(wallDirection)*10/diw
	    ai.desiredV = ai.desiredSpeed*ai.direction
